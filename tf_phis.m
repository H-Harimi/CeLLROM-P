% function [phis_tf,D,res0,cellData] = tf_phis(s,locs,cellData,electrode) 
%
%   Returns the frequency response of the Phi_s(locs,s)/I_app(s) 
...
function [phis_tf,Dterm,res0,cellData] = tf_phis(s,locs,cellData,electrode)
  ...
  if(max(locs)>1 || min(locs)<0)
    error('ERROR (tf_phis): All values of "locs" must be in [0,1]');
  end
  ...
  elseif(strcmpi(electrode,'pos'))
    ...
    error('ERROR (tf_phis): "electrode" must be "neg" or "pos"');
  end
  ...
  phis_tf = zeros(len,length(s)); % Initialize output to zero
  % Initialize "D" variables and transfer-function names variable
  Dterm = zeros(len,1); Dstr = cell(len,1); names = cell(len,1);
  for n=1:len
    z = locs(n);
    phis_tf(n,:) = -L*(kappa_eff*(cosh(nu) - cosh((z-1).*nu)) +...
                      sigma_eff*(1-cosh(z.*nu) + z.*nu.*sinh(nu))) ...
                      ./(Acell*sigma_eff*(kappa_eff + sigma_eff)*...
                      nu.*sinh(nu));
    Dterm(n) = -L*(kappa_eff*(cosh(nu_inf) - cosh((z-1).*nu_inf)) +...
                      sigma_eff*(1-cosh(z.*nu_inf)+z.*nu_inf.*sinh(nu_inf)))...
                      ./(Acell*sigma_eff*(kappa_eff + sigma_eff)*...
                      nu_inf.*sinh(nu_inf)); 
    if(strcmpi(elec.name,'neg'))
      Dstr{n} = sprintf(['-%g*(kappa_eff_neg*(cosh(nu_neg)-'...
                          'cosh((%g-1).*nu_neg))+sigma_eff_neg*'...
                          '(1-cosh(%g.*nu_neg)+%g.*nu_neg.*'...
                          'sinh(nu_neg)))./(%g*sigma_eff_neg*'...
                          '(kappa_eff_neg+sigma_eff_neg)*nu_neg.*'...
                          'sinh(nu_neg))'],L,z,z,z,Acell);
    else
      Dstr{n} = sprintf(['%g*(kappa_eff_pos*(cosh(nu_pos)-'...
                          'cosh((%g-1).*nu_pos))+sigma_eff_pos*'...
                          '(1-cosh(%g.*nu_pos)+%g.*nu_pos.*'...
                          'sinh(nu_pos)))./(%g*sigma_eff_pos*'...
                          '(kappa_eff_pos+sigma_eff_pos)*nu_pos.*'...
                          'sinh(nu_pos))'],L,z,z,z,Acell);
    end    
    names{n} = sprintf('phis_%s',electrode);
    
    % value of tf at s->0. Solved with Mathematica.
    tf0 = L*(locs-2).*locs/(2*Acell*sigma_eff); 
    phis_tf(:,s==0) = tf0(:,ones(size(find(s==0))));

    if any(isnan(phis_tf)),
      error('ERROR (tf_phis): At least one value computes to NaN');
    end    
  end
