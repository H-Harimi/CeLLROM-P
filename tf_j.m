% function [j_tf,D,res0,cellData] = tf_j(s,locs,cellData,electrode) 
%
%   Returns the frequency response of the J(locs,s)/I_app(s) 
...
%
%   Outputs:
%     j_tf: (complex) freq response at requested locations, freqs
...
function [j_tf,Dterm,res0,cellData] = tf_j(s,locs,cellData,electrode)
  ...
  if(max(locs)>1 || min(locs)<0)
    error('ERROR (tf_j): All values of "locs" must be in [0,1]');
  end
  ...
  elseif(strcmpi(electrode,'pos'))
    ...
    error('ERROR (tf_j): "electrode" must be "neg" or "pos"');
  end
  ...
  for n=1:len,
    ...
    Dterm(n)  = nu_inf./(as*F*L*Acell*(kappa_eff+sigma_eff).*...
                 sinh(nu_inf)).*(sigma_eff*cosh(nu_inf.*z) ...
                 + kappa_eff*cosh(nu_inf.*(z-1)));
    if(strcmpi(elec.name,'neg'))
      Dstr{n} = sprintf(['nu_neg./(%g*(kappa_eff_neg+sigma_eff_neg).*'...
              'sinh(nu_neg)).*(sigma_eff_neg*cosh(nu_neg.*%g)'...
              '+ kappa_eff_neg*cosh(nu_neg.*(%g-1)))'],as*F*L*Acell,z,z); 
    else
      Dstr{n} = sprintf(['-nu_pos./(%g*(kappa_eff_pos+sigma_eff_pos).*'...
              'sinh(nu_pos)).*(sigma_eff_pos*cosh(nu_pos.*%g)'...
              '+ kappa_eff_pos*cosh(nu_pos.*(%g-1)))'],as*F*L*Acell,z,z); 
    end
    names{n} = sprintf('j_%s',electrode);
    if any(isnan(j_tf)),
  ...
  res0 = zeros(len,1); % Every location has same res0
