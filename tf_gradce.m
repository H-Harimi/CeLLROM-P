% function [ce_tf,D,res0,cellData] = tf_gradce(s,locs,cellData,M) 
%
%   Returns the frequency response of the Nabla C_e(locs,s)/I_app(s) 
...
%
%   Outputs:
%     gradce_tf: (complex) frequency response at requested locations, freqs
...
function [gradce_tf,Dterm,res0,cellData] = tf_gradce(s,locs,cellData,M)
  ...
  if(min(locs) < 0 || max(locs) > Ltot+eps)
    error('ERROR (tf_gradce): "x" must be in range 0 to %2.2e',Ltot);
  end
  ...
  % Move some variables from cellData structure into more convenient local
  % variable definitions
  ...
  gradce_tf = zeros(len,length(s)); % Initialize output to zeros
  ...
  for n = 1:M,
    ...
    for theLoc = 1:len, % Now, weight by eigenfunction
      x = locs(theLoc); 
      if x <= Lneg+eps, % Compute gradient of the eigenfunction at this loc
        Psi = -k1*n1*sin(n1*x); % negative electrode region
      elseif x >= Lnegsep - eps,
        Psi = -k5*n3*sin(n3*x) + k6*n3*cos(n3*x);% pos. electrode
      else
        Psi = -k3*n2*sin(n2*x) + k4*n2*cos(n2*x); % separator
      end
      % Add the contribution of this eigenvalue to what has already been
      % computed
      gradce_tf(theLoc,:) = gradce_tf(theLoc,:) + Psi*ce_n.';
